...
import htsjdk.samtools.util.{BlockCompressedInputStream, Interval, OverlapDetector}
...
class VCFFileFormat extends TextBasedFileFormat with DataSourceRegister with HlsEventRecorder {
  ...
  override def buildReader(
    ...
    val filteredContigAndInterval =
      TabixIndexHelper.makeFilteredContigAndInterval(filters, useFilterParser, useIndex)
        ...
        case Some((startPos, endPos)) =>
          ...
          SchemaDelegate
            ...
            .toRows(
              header,
              requiredSchema,
              VCFIteratorDelegate.makeDelegate(options, reader, codec, filteredContigAndInterval))
      }
    }
...
private object VCFIteratorDelegate {
  def makeDelegate(
      ...
      codec: VCFCodec,
      filteredContigAndInterval: ContigAndInterval): AbstractVCFIterator = {
    if (options.get(VCFOptions.SPLIT_TO_BIALLELIC).exists(_.toBoolean)) {
      new SplitVCFIterator(new VCFIterator(lineReader, codec, filteredContigAndInterval))
    } else {
      new VCFIterator(lineReader, codec, filteredContigAndInterval)
    }
  }
}
...
private[vcf] class VCFIterator(
    lineReader: Iterator[Text],
    codec: VCFCodec,
    filteredContigAndInterval: ContigAndInterval)
    extends AbstractVCFIterator(codec)
    with GlowLogging {
  // filteredContigAndInterval contains the contig and interval generated by
  ...
  private val overlapDetector: OverlapDetector[Interval] =
    if (!filteredContigAndInterval.contig.isAnyContig) {
      OverlapDetector.create(
        scala
          .collection
          .immutable
          .List[Interval](
            filteredContigAndInterval.toHTSJDKInterval
          )
          .asJava
      )
    } else {
  ...
  private def overlaps(vc: VariantContext): Boolean = {
    ...
    if (filteredContigAndInterval.contig.isAnyContig) {
      // anyContig means that the filter parser is not being used due to multiple contigs
      // being queried or filter parser being disabled by the user.
      true
    } else {
      val vcInterval = new Interval(vc.getContig, vc.getStart, vc.getEnd)
