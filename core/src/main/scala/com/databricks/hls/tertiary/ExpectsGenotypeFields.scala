...
trait ExpectsGenotypeFields extends Expression {
  ...
  protected lazy val genotypeFieldIndices: Seq[Int] = {
    val gStruct = genotypesExpr.dataType.asInstanceOf[ArrayType].elementType
  ...
  override def checkInputDataTypes(): TypeCheckResult = {
    ...
    if (!adt.elementType.isInstanceOf[StructType]) {
      return TypeCheckResult.TypeCheckFailure("Genotypes field must be an array of structs")
    }

    val missingFields = genotypeFieldsRequired.zip(genotypeFieldIndices).collect { case (f, -1) =>
      f
    }
