...
class LiftOverExprSuite extends HLSBaseTest {
  ...
  private def liftOverAndCompare(
    ...
    val outputDf = if (minMatchOpt.isDefined) {
      inputDf
        ...
        .withColumn(
          "lifted",
          expr(s"lift_over(contigName, start, end, '$chainFile', minMatchRatio)"))
        .drop("minMatchRatio")
    } else {
      inputDf
        .withColumn("lifted", expr(s"lift_over(contigName, start, end, '$chainFile')"))
    }
    val liftedDf =
      outputDf
  ...
  private def readVcf(vcfFile: String): DataFrame = {
    spark.read.format("com.databricks.vcf").load(vcfFile).select("contigName", "start", "end")
  }
  ...
  test("Basic") {
    compareLiftedVcf(
      ...
      s"$testDataHome/liftover/lifted.combined.chr20_18210071_18210093.g.vcf.unmap",
      None
    )
  }
  ...
  test("Some failures") {
    compareLiftedVcf(
      ...
      s"$testDataHome/liftover/lifted.test.vcf.unmap",
      None)
  }
  ...
  test("High min match") {
    compareLiftedBed(
      ...
      s"$testDataHome/liftover/lifted.minMatch05.test.bed.unmap",
      Some(0.5)
    )
  }
  ...
  test("Low min match") {
    compareLiftedBed(
      ...
      s"$testDataHome/liftover/lifted.minMatch001.test.bed.unmap",
      Some(0.01)
    )
  }
  ...
  test("Do not cache chain file") {
    ...
    inputDf.selectExpr(s"lift_over(contigName, start, end, '$chainFile')").collect()
    val ex = intercept[SparkException](
      inputDf.selectExpr("lift_over(contigName, start, end, 'fakeChainFile')").collect()
    )
  ...
  test("Null contigName") {
    ...
    val outputDf =
      inputDf.withColumn("lifted", expr(s"lift_over(null, start, end, '$chainFile')"))
  ...
  test("Null start") {
    val inputDf =
      readBed(s"$testDataHome/liftover/unlifted.test.bed")
    val outputDf =
      inputDf.withColumn("lifted", expr(s"lift_over(contigName, null, end, '$chainFile')"))
    assert(outputDf.filter("lifted is null").count == outputDf.count)
  }
  ...
  test("Null end") {
    val inputDf =
      readBed(s"$testDataHome/liftover/unlifted.test.bed")
    val outputDf =
      inputDf.withColumn("lifted", expr(s"lift_over(contigName, start, null, '$chainFile')"))
    assert(outputDf.filter("lifted is null").count == outputDf.count)
  }
  ...
  test("Null minMatchRatio") {
    val inputDf =
      readBed(s"$testDataHome/liftover/unlifted.test.bed")
    val outputDf =
      inputDf.withColumn("lifted", expr(s"lift_over(contigName, start, end, '$chainFile', null)"))
    assert(outputDf.filter("lifted is null").count == outputDf.count)
  }
}
