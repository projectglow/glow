...
import com.databricks.vcf.VCFConverterBaseTest
...
class LiftOverVariantsTransformerSuite extends HLSBaseTest with VCFConverterBaseTest {
  ...
  test("Missing chain file") {
    ...
    val ex = intercept[SparkException] {
      val outputDf =
        DBGenomics.transform("lift_over_variants", inputDf, Map("referenceFile" -> REFERENCE_FILE))
      outputDf.count
    }
  ...
  test("Missing reference file") {
    ...
    val ex = intercept[SparkException] {
      val outputDf =
        DBGenomics.transform("lift_over_variants", inputDf, Map("chainFile" -> CHAIN_FILE))
      outputDf.count
    }
